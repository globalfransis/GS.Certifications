      ...
      thead(ref='head')
        tr.headers-row(:style='{ "grid-template-columns": gridTemplateColumns }')
          th(
            ...
            @mousedown='sort(column)'
          )
            span.header-content {{ column.headerName }}
            span.resize-handle(@mousedown='initResize(column, $event)')
...
export default {
    ...
    document.addEventListener('keydown', $event => {
      ...
      if (key === 'ArrowDown') {
        ...
        $event.preventDefault()
          ...
          if (this.columnDefs[colIndex].type == 'select') {
  ...
  computed: {
    ...
    rowDataPage () {
      ...
      return this.rowDataFiltered.slice(this.page * this.pageC, this.page * this.pageC + this.pageC)
    },
    columnDefsFiltered () {
      return this.columnDefs
    },
    pages () {
      return Math.ceil(this.rowDataFiltered.length / this.pageC)
    }
  },
  methods: {
    emitRowSelected () {
